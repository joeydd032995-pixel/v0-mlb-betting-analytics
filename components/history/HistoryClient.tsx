...
export function HistoryClient({ dbPredictions, dbTotalAvailable, dbCap }: Props) {
  ...
  const handleRecordResult = (id: string, homeRuns: number, awayRuns: number) => {
    ...
    setActionError(null)
    // The action resolves with `{ ok: false }` rather than throwing when the row
    ...
    recordResultAction(id, homeRuns, awayRuns)
      ...
      .catch((err: unknown) => {
        console.error(err)
        setActionError("Saved on this device only — couldn't reach the server to record the result.")
      })
  }
  ...
  return (
    <>
      {actionError && (
        <div
          className="rounded-[10px] border px-3 py-2 font-jet text-[11px] leading-relaxed"
          style={{
      ...
      <MethodologyPanel
        ...
        caveats={[
          ...
          "Backtested predictions use point-in-time pitcher and team stats but synthetic month-average weather and no odds, which pulls measured accuracy toward the raw league NRFI rate — diluting the figures above rather than inflating them.",
