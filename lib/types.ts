...
export const METRIC_GLOSSARY: Record<string, string> = {
  ...
  confidence: "The tier shown on a prediction (High / Medium / Low). On predictions made since the 2026-08 retier it is driven by CONVICTION — how far the NRFI probability sits from a coin flip, measured as |p − 50%| × 2 (High at 0.18 and up, Medium at 0.11 and up) — so it says how decisive the call is, not how much data is behind it. Older rows in your history keep the tier they were saved with, assigned by the previous rule off the reliability score, so a High on an archived prediction does not imply that conviction cutoff",
  confidenceScore: "The number beside the confidence tier (10–98): a reliability score for the INPUTS, not a forecast of winning. It starts at 50 and moves with starter first-inning sample size, how stable both starters' recent form has been, and simulation variance. Measured against outcomes across 3,444 verified predictions it carried no signal — the gap between its top and bottom quintile was −0.006 hit rate (z = −0.22) — so it should not be used to rank or compare picks. A high tier next to a middling score means a decisive call built on thin or noisy inputs",
